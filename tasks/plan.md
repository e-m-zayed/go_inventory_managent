...
| 08 — Operations & QR + Barcode Scan | Cold chain fields, CycleCount, StockTransfer bulk, RecallLot, QR generator + scan route, browser-side barcode scanning (`BarcodeDetector` + vendored `jsQR` fallback) | 03, 07 | QR scans to the right entity; camera-scan page decodes a QR and lands on the entity; cycle count variance report correct |
...
- **Barcode scanning browser variance.** Mitigation: feature-detect `BarcodeDetector` first; fall back to vendored `jsQR`. The decode→POST→`/{lang}/scan/{code}` flow is browser-agnostic — both decoders produce the same string the server route already accepts.
